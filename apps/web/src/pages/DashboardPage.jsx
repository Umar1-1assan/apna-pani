...
import { LayoutDashboard, Settings, Building2, Receipt, Lock, MapPin, Package, LogOut, CalendarDays, Users, Bike, Truck, Plus, Banknote } from "lucide-react";
  ...
  switch (role) {
    ...
    case "customer":
      ...
      navItems = [
        ...
        { id: "profile", label: t("profile_settings"), icon: <Settings size={20} /> }
      ];
      break;
    case "supplier":
    default:
      ...
      navItems = [
        ...
        { id: "config", label: t("configuration"), icon: <Settings size={20} /> }
      ];
      break;
  }

  return (
    <div className="min-h-screen bg-[#f8f9ff] text-[#0b1c30] flex flex-col md:flex-row font-sans antialiased">
      ...
      <nav className="md:hidden fixed bottom-0 w-full z-50 flex justify-around items-center px-2 pb-4 pt-2.5 bg-white border-t border-[#dce9ff] shadow-lg rounded-t-2xl overflow-x-auto">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => setActiveTab(item.id)}
            className={`flex flex-col items-center justify-center text-[10px] gap-1 px-2 ${
              activeTab === item.id ? "text-blue-600 font-bold" : "text-gray-400"
            }`}
          >
            {item.icon}
        ...
        <button
          ...
          className="flex flex-col items-center justify-center text-red-400 text-[10px] gap-1 px-2"
        >
          <LogOut size={20} />
          <span>Logout</span>
        </button>
      ...
      <main className="flex-1 md:ml-64 flex flex-col min-h-screen pb-24 md:pb-6 bg-[#f8f9ff]">
